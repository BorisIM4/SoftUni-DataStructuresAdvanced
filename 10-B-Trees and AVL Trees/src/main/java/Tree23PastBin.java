    ...
    static class Tree23<K extends Comparable<K>> {
        static class Node23<K extends Comparable<K>> {
            ...
            public String toString() {
                return "{" + String.join(",", keys.stream().map(Object::toString).collect(Collectors.toList())) + "}";
            }
            ...
            public Node23<K> insert(K key) {
                ...
                while (insertionNode.keys.size() == 3) {
                    Node23<K> leftPart = new Node23<>(insertionNode.parent, insertionNode.keys.get(0),
                            !insertionNode.subNodes.isEmpty() ? insertionNode.subNodes.subList(0, 2) : new ArrayList<>());
                    Node23<K> rightPart = new Node23<>(insertionNode.parent, insertionNode.keys.get(2),
                            !insertionNode.subNodes.isEmpty() ? insertionNode.subNodes.subList(2, 4) : new ArrayList<>());
                    ...
                    if (insertionNode.parent != null) {
                        ...
                        insertionNode = new Node23<K>(null, median, Arrays.asList(leftPart, rightPart));
                    }
                }
        ...
        public void add(K key) {
            if (root == null) {
                root = new Node23<K>(null, key, Collections.emptyList());
            } else {
